...
# Visualisasi interaktif tambahan berdasarkan pertanyaan bisnis
st.subheader("📦 Kategori Produk Paling Populer")
product_counts = filtered_data['product_category_name'].value_counts().reset_index()
product_counts.columns = ['category', 'count']
fig_category = px.bar(product_counts.head(10), x='category', y='count', title='Top 10 Kategori Produk', color='count', color_continuous_scale='viridis')
st.plotly_chart(fig_category, use_container_width=True)

st.subheader("🚚 Rata-rata Waktu Pengiriman per Negara")
delivery_time = filtered_data.groupby('customer_state')['order_delivered_customer_date'].count().reset_index()
delivery_time.columns = ['state', 'count']
fig_delivery = px.bar(delivery_time, x='state', y='count', title='Jumlah Pengiriman per Negara', color='count', color_continuous_scale='blues')
