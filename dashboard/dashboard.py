...
def load_data():
    try:
        all_data = pd.read_csv("data/all_data.csv", parse_dates=["order_purchase_timestamp"])
        return all_data
    except FileNotFoundError:
        st.error("File all_data.csv tidak ditemukan. Pastikan file tersedia di direktori kerja.")
        return pd.DataFrame()
